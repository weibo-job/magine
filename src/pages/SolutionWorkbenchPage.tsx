...
export default function SolutionWorkbenchPage({ artifact, onBack, onContinueDiscussion }: Props) {
  ...
  async function generatePreview() {
    ...
    try {
      const prompt = `你是资深网页设计师。根据下面的页面方案生成一个可直接放进 iframe srcDoc 的完整 HTML 页面。要求：只输出完整 HTML，不要 markdown 代码围栏；所有 CSS 写在 style 标签内；不要外部图片、脚本、字体或网络资源；用渐变、色块和占位结构表达视觉层级；实现页面主要交互的静态状态和克制的 hover / loading 动效；页面要有真实产品感，不要只生成线框。\n\n页面方案：\n${artifact.answer}\n\n当前迭代要求：\n${iterationNote || '这是第一次生成，请按方案完成一版可看的页面 Demo。'}`
      const result = await deepseekChat(key, [{ role: 'user', content: prompt }], getKey('deepseek_model') || undefined)
  ...
  return <main className="solution-workbench">
    ...
    <div className="solution-layout">
      <section className="solution-spec"><div className="solution-section"><span className="eyebrow">BRIEF</span><h2>目标与讨论结论</h2><pre>{artifact.answer}</pre></div>{page && <><div className="solution-section"><span className="eyebrow">STRUCTURE</span><h2>页面结构</h2><pre>{section(artifact.answer, '页面结构')}</pre></div><div className="solution-section"><span className="eyebrow">COMPONENTS</span><h2>组件清单</h2><pre>{section(artifact.answer, '组件清单')}</pre></div><div className="solution-section"><span className="eyebrow">INTERACTION</span><h2>交互与动效</h2><pre>{section(artifact.answer, '交互流程')}\n\n{section(artifact.answer, '动效方案')}</pre></div></>}</section>
